...
//---------------------------------------------------------
// Shader:   DesertMorning.glsl      by ESpitz 7/2013
//           
// original: https://www.shadertoy.com/view/lsf3zB
// tags: procedural, 3d, distancefield, terrain
//   A second attempt at procedural terrain generation,
//   based on techniques developd by Inigo Quilez 
//---------------------------------------------------------
#ifdef GL_ES
...
vec3 render( vec3 ro, vec3 rd ) {
	...
	if (res.y>-0.5 && res.y<0.5) {
		...
		color = mix( vec3(0.6, 0.5, 0.1), color, n*smoothstep(0.0, 0.7, nor.y) ); 
...
void main( void ) 
{
	...
	float y = 0.0;
	float z = .0 + sin(0.001*time);
	vec3 cPos = vec3(x, y, z); // position
	cPos.y = terrain(cPos.xz, 1) + 1.5+5.0;
	
	const vec3 cUp = vec3(0., 1., 0.); // up 
	vec3 cLook = vec3(cPos.x + 2.0, cPos.y*-1., -0.0); // lookAt
	...
	vec3 rd = normalize( pos.x*uu + pos.y*vv-1.0 + 2.0*ww );
